-- Moderators of a live room, next to the invisible trusted participants.
--
-- A trusted participant stays invisible and only has the messages they write
-- approved as they are written. A moderator is announced by a badge in the
-- room and moderates it from the chat itself: approving, rejecting, correcting
-- and pinning messages, trusting their authors, and taking the interactions of
-- an author away. Appointing another moderator deliberately stays with the
-- administration alone, so a room can never grow its own moderators.
...
-- Every load of a chat asks who moderates it, so the handful of moderators of
-- a room is indexed instead of being looked for in its whole audience.
CREATE INDEX IF NOT EXISTS workshop_participants_moderator_idx
    ON public.workshop_participants (workshop_id)
    WHERE is_moderator;

-- The paged participant administration returns the moderator and filters by
-- them. The previous signature is dropped rather than replaced, because an
-- added parameter would otherwise leave a second overload of the very same
-- name behind for a request to choose from.
