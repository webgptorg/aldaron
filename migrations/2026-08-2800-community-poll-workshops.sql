...
-- Only a workshop occurrence is a subject a poll can be about. The community room itself already owns its polls, so
-- attaching it to them would say nothing and would let one poll be listed twice in the very same place.
CREATE OR REPLACE FUNCTION public.enforce_workshop_poll_workshop_kind()
RETURNS trigger
LANGUAGE plpgsql
...
CREATE TRIGGER workshop_poll_workshops_enforce_kind
    BEFORE INSERT OR UPDATE OF workshop_id ON public.workshop_poll_workshops
    FOR EACH ROW EXECUTE FUNCTION public.enforce_workshop_poll_workshop_kind();
...
BEGIN
    ...
    SELECT DISTINCT target_poll_id, attached.workshop_id
    FROM unnest(target_attached_workshop_ids) AS attached(workshop_id)
...
REVOKE ALL ON FUNCTION public.enforce_workshop_poll_workshop_kind() FROM PUBLIC, anon, authenticated;
...
REVOKE ALL ON FUNCTION public.update_community_workshop_poll(uuid, uuid, text, jsonb, boolean, boolean, uuid[])
    FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.enforce_workshop_poll_workshop_kind() TO service_role;
