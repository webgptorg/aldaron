-- The participant page of the workshop administration, ordered by columns which name their row source.
--
-- `RETURNS TABLE` declares every returned column as a PL/pgSQL variable of the
-- function, so the unqualified `fullname` of the previous ordering could mean
-- either that variable or the column of the same name. PostgreSQL refuses such
-- a query with `column reference "fullname" is ambiguous` while it prepares it,
-- which took down the participants of `/admin/workshops` and their CSV and
-- vCard exports whatever the chosen sort was, because the whole ordering is
-- parsed before a single row is read.
--
-- Every ordering key therefore names the row source it sorts by, exactly like
-- the filters of the very same query already do. Nothing else changes: a
-- qualified reference resolves to the column it already resolved to.
