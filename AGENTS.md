...
- `/cs/komunita` - The Czech-only room of the one permanent community, built on the very same participant room. Being a `community` room rather than a workshop occurrence, it has no schedule, no stage, and no live updates, so it offers the chat, the polls of the community, the materials, and links into every published workshop. A poll which is about workshop occurrences names the published ones as links into their rooms. Each of those links carries the `email` and `fullname` of the member reading the room, so the workshop room they lead to prefills its connection form instead of asking for either again.
