...
- `/cs/komunita/clenstvi` - The Czech public landing page for Basic, Standard and Premium community membership, focused on Premium for developers, creators and small business owners. Basic is free, Standard costs 150 CZK per month and Premium costs 1000 CZK per month; both paid plans have a seven-day trial, default to annual billing with a 20% discount, and accept the shared discount codes after that reduction. The page reads `fullname`, `email` and `code` from its query, shows every price as a monthly equivalent, and saves a server-calculated membership application into contacts because this app has no payment-provider integration; activation is sent separately by e-mail.
