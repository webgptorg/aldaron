...
- `/cs/komunita` - The Czech-only room of the one permanent community, built on the very same participant room. Being a `community` room rather than a workshop occurrence, it has no schedule, no stage, and no live updates, so it offers the chat, the materials, and links into every published workshop. Each of those links carries the `email` and `fullname` of the member reading the room, so the workshop room they lead to prefills its connection form instead of asking for either again. It also offers the projects of the members, where a project is browsed by its category, liked, and shared under the name the room already knows the member by; those projects are mock data kept in the browser and are not stored anywhere yet.
