    ...
    return (
        ...
        >
            ...
            >
                ...
                <div className={`shrink-0 w-8 h-8 rounded-full bg-gray-50 border border-gray-100 flex items-center justify-center transition-all duration-300 ${isOpen ? 'rotate-180 bg-cyan-50 border-cyan-200' : ''}`}>
                    <ChevronDown className={`w-4 h-4 transition-colors duration-200 ${isOpen ? 'text-cyan-600' : 'text-gray-400'}`} />
                </div>
            </button>
            <div
                className={`overflow-hidden transition-all duration-300 ease-in-out ${isOpen ? 'max-h-[500px] opacity-100 pb-6' : 'max-h-0 opacity-0'}`}
            >
                <p className="text-[15px] text-gray-500 leading-relaxed pr-12">
                    {answer}
                </p>
            </div>
        </motion.div>
