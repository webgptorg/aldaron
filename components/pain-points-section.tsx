...
import { FolderSearch, ShieldAlert, UserMinus, Users } from 'lucide-react';
                ...
                <div className="grid md:grid-cols-2 gap-5">
                    {painPoints.map((point, i) => (
                        ...
                        >
                            {/* Top accent line */}
                            <div
                                className={`absolute top-0 left-8 right-8 h-px bg-gradient-to-r ${point.accentColor} opacity-0 group-hover:opacity-40 transition-opacity duration-500`}
                            ></div>

                            <div className="flex gap-5">
                                {/* Icon */}
                                <div
                                    className={`shrink-0 w-12 h-12 rounded-xl ${point.iconBg} flex items-center justify-center group-hover:scale-110 transition-transform duration-300`}
                                >
                                ...
                                <div className="space-y-3 min-w-0">
                                    <h3 className="text-lg font-bold text-[#0f172a] tracking-tight">{point.title}</h3>
                                    <p className="text-[15px] text-gray-500 leading-relaxed">{point.description}</p>
                                    {/* Consequence tag */}
                                    <div
                                        className={`inline-flex items-start gap-2 bg-gradient-to-r ${point.bgAccent} rounded-lg px-3.5 py-2.5 border ${point.borderAccent}`}
                                    >
