...
- Added a menu next to the CSV and the vCard button of the contacts dashboard, which opens the export in a new tab instead of downloading it, while clicking the button itself downloads the file as before. The opened address carries the whole filter and sorting of the view and is opened by the very same token as the dashboard, so reloading that tab exports the contacts as they are at that later moment.
